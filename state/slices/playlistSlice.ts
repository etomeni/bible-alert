  ...
  reducers: {
    ...
    editPlaylist: (state, action: PayloadAction<_editPlaylistInterface_>) => {
      ...
      const indexOfObject = state.findIndex(
        (obj) => obj.title == action.payload.oldTitle
      );
      if (indexOfObject !== -1) {
    ...
    schedulePlaylist: (
      state,
      action: PayloadAction<_schedulePlaylistInterface_>
    ) => {
      // console.log(action.payload);
      const indexOfObject = state.findIndex(
    ...
    addToPlaylist: (state, action: PayloadAction<_playlistInterface_>) => {
      ...
      if (indexOfObject !== -1) {
        ...
        const updatedInnerArray = updatedArray[indexOfObject].lists.filter(
          (item) =>
            item.book !== action.payload.bibleVerse.book ||
            item.chapter !== action.payload.bibleVerse.chapter ||
            item.verse !== action.payload.bibleVerse.verse
        );
